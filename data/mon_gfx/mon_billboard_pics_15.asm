TyroguePic:
	INCBIN "gfx/billboard/mon_pics/tyrogue.2bpp"
TyrogueSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/tyrogue.2bpp"
HitmontopPic:
	INCBIN "gfx/billboard/mon_pics/hitmontop.2bpp"
HitmontopSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/hitmontop.2bpp"
SmoochumPic:
	INCBIN "gfx/billboard/mon_pics/smoochum.2bpp"
SmoochumSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/smoochum.2bpp"
ElekidPic:
	INCBIN "gfx/billboard/mon_pics/elekid.2bpp"
ElekidSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/elekid.2bpp"
MagbyPic:
	INCBIN "gfx/billboard/mon_pics/magby.2bpp"
MagbySilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/magby.2bpp"
MiltankPic:
	INCBIN "gfx/billboard/mon_pics/miltank.2bpp"
MiltankSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/miltank.2bpp"
BlisseyPic:
	INCBIN "gfx/billboard/mon_pics/blissey.2bpp"
BlisseySilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/blissey.2bpp"
RaikouPic:
	INCBIN "gfx/billboard/mon_pics/raikou.2bpp"
RaikouSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/raikou.2bpp"
EnteiPic:
	INCBIN "gfx/billboard/mon_pics/entei.2bpp"
EnteiSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/entei.2bpp"
SuicunePic:
	INCBIN "gfx/billboard/mon_pics/suicune.2bpp"
SuicuneSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/suicune.2bpp"
LarvitarPic:
	INCBIN "gfx/billboard/mon_pics/larvitar.2bpp"
LarvitarSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/larvitar.2bpp"
PupitarPic:
	INCBIN "gfx/billboard/mon_pics/pupitar.2bpp"
PupitarSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/pupitar.2bpp"
TyranitarPic:
	INCBIN "gfx/billboard/mon_pics/tyranitar.2bpp"
TyranitarSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/tyranitar.2bpp"
LugiaPic:
	INCBIN "gfx/billboard/mon_pics/lugia.2bpp"
LugiaSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/lugia.2bpp"
HoOhPic:
	INCBIN "gfx/billboard/mon_pics/ho_oh.2bpp"
HoOhSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/ho_oh.2bpp"
CelebiPic:
	INCBIN "gfx/billboard/mon_pics/celebi.2bpp"
CelebiSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/celebi.2bpp"
TreeckoPic:
	INCBIN "gfx/billboard/mon_pics/treecko.2bpp"
TreeckoSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/treecko.2bpp"
GrovylePic:
	INCBIN "gfx/billboard/mon_pics/grovyle.2bpp"
GrovyleSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/grovyle.2bpp"
SceptilePic:
	INCBIN "gfx/billboard/mon_pics/sceptile.2bpp"
SceptileSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/sceptile.2bpp"
TorchicPic:
	INCBIN "gfx/billboard/mon_pics/torchic.2bpp"
TorchicSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/torchic.2bpp"
CombuskenPic:
	INCBIN "gfx/billboard/mon_pics/combusken.2bpp"
CombuskenSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/combusken.2bpp"
