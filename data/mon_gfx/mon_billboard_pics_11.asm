ChikoritaPic:
	INCBIN "gfx/billboard/mon_pics/chikorita.2bpp"
ChikoritaSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/chikorita.2bpp"
BayleefPic:
	INCBIN "gfx/billboard/mon_pics/jigglypuff.2bpp"
BayleefSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/jigglypuff.2bpp"
MeganiumPic:
	INCBIN "gfx/billboard/mon_pics/meganium.2bpp"
MeganiumSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/meganium.2bpp"
CyndaquilPic:
	INCBIN "gfx/billboard/mon_pics/cyndaquil.2bpp"
CyndaquilSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/cyndaquil.2bpp"
QuilavaPic:
	INCBIN "gfx/billboard/mon_pics/quilava.2bpp"
QuilavaSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/quilava.2bpp"
TyphlosionPic:
	INCBIN "gfx/billboard/mon_pics/jigglypuff.2bpp"
TyphlosionSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/jigglypuff.2bpp"
TotodilePic:
	INCBIN "gfx/billboard/mon_pics/totodile.2bpp"
TotodileSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/totodile.2bpp"
CroconawPic:
	INCBIN "gfx/billboard/mon_pics/croconaw.2bpp"
CroconawSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/croconaw.2bpp"
FeraligatrPic:
	INCBIN "gfx/billboard/mon_pics/feraligatr.2bpp"
FeraligatrSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/feraligatr.2bpp"
SentretPic:
	INCBIN "gfx/billboard/mon_pics/sentret.2bpp"
SentretSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/sentret.2bpp"
FurretPic:
	INCBIN "gfx/billboard/mon_pics/furret.2bpp"
FurretSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/furret.2bpp"
HoothootPic:
	INCBIN "gfx/billboard/mon_pics/hoothoot.2bpp"
HoothootSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/hoothoot.2bpp"
NoctowlPic:
	INCBIN "gfx/billboard/mon_pics/noctowl.2bpp"
NoctowlSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/noctowl.2bpp"
LedybaPic:
	INCBIN "gfx/billboard/mon_pics/ledyba.2bpp"
LedybaSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/ledyba.2bpp"
LedianPic:
	INCBIN "gfx/billboard/mon_pics/jigglypuff.2bpp"
LedianSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/jigglypuff.2bpp"
SpinarakPic:
	INCBIN "gfx/billboard/mon_pics/jigglypuff.2bpp"
SpinarakSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/jigglypuff.2bpp"
AriadosPic:
	INCBIN "gfx/billboard/mon_pics/ariados.2bpp"
AriadosSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/ariados.2bpp"
CrobatPic:
	INCBIN "gfx/billboard/mon_pics/crobat.2bpp"
CrobatSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/crobat.2bpp"
ChinchouPic:
	INCBIN "gfx/billboard/mon_pics/chinchou.2bpp"
ChinchouSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/chinchou.2bpp"
LanturnPic:
	INCBIN "gfx/billboard/mon_pics/lanturn.2bpp"
LanturnSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/lanturn.2bpp"
PichuPic:
	INCBIN "gfx/billboard/mon_pics/pichu.2bpp"
PichuSilhouettePic:
	INCBIN "gfx/billboard/mon_silhouettes/pichu.2bpp"
