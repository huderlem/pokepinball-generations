ResolveRubyFieldTopGameObjectCollisions: ; 0x1460e
	call ResolveVoltorbCollision_RubyField
	call ResolveRubyStageSpinnerCollision
	call ResolveBallUpgradeTriggersCollision_RubyField
	call UpdateBallTypeUpgradeCounter_RubyField
	...
	call UpdateBlinkingPokeballs_RubyField
	call UpdateMapMoveCounters_RubyFieldTop
	callba ShowExtraBallMessage
	ld a, $0
	callba CheckSpecialModeColision
	ret

ResolveRubyFieldBottomGameObjectCollisions: ; 0x14652
	call ResolveWildMonCollision_RubyField
	call ResolveBumpersCollision_RubyField
	call ResolveDiglettCollision_RubyField
	call UpdateMapMoveCounters_RubyFieldBottom
	call UpdateRubyStageSpinner
	call UpdatePinballUpgradeBlinkingAnimation_RubyField
	call UpdateBallTypeUpgradeCounter_RubyField
...
ResolveDiglettCollision_RubyField: ; 0x147aa
	ld a, [wWhichDiglett]
	and a
	...
	ld a, [wRightMapMoveCounter]
	cp $3
	call z, HitRightDiglett3Times_RubyField
	jr .asm_14830
...
.asm_14807
	...
	ld a, [wLeftMapMoveCounter]
	cp $3
	call z, HitLeftDiglett3Times_RubyField
.asm_14830
	call AddScoreForHittingDiglett_RubyField
	ret
...
.asm_14872
	...
	ld [wStageCollisionMap + $110], a
.asm_1487c
	call UpdateDiglettAnimations_RubyField
	ret

UpdateMapMoveCounters_RubyFieldBottom: ; 0x14880
...
UpdateMapMoveCounters_RubyFieldTop: ; 0x148cf
; This is identical logic to UpdateMapMoveCounters_RubyFieldBottom, but it doesn't load
; any graphics, since the Diglett aren't in the Top half of the Ruby Field.
	ld b, $0
...
HitRightDiglett3Times_RubyField: ; 0x14920
...
HitLeftDiglett3Times_RubyField: ; 0x14947
	ld hl, wNumDugtrioTriples
	call Increment_Max100
	jr nc, .asm_1495e
	ld c, $a
...
AddScoreForHittingDiglett_RubyField: ; 0x1496d
	ld a, $55
	ld [wRumblePattern], a
	ld a, $4
...
UpdateDiglettAnimations_RubyField: ; 0x14990
	ld a, [wLeftDiglettAnimationController]
	and a
	jr nz, .asm_149b6
...
_LoadDiglettGraphics_RubyField: ; 0x149d9
	sla a
	ld c, a
	ld b, $0
...
.asm_149e9
	...
	or h
	ret z
	ld a, BANK(TileListDataPointers_14a11_RubyField)
	call QueueGraphicsToLoad
	ret

LoadDiglettNumberGraphics_RubyField: ; 0x149f5
	sla a
	ld c, a
	ld b, $0
...
.asm_14a05
	...
	or h
	ret z
	ld a, BANK(Data_14af5_RubyField)
	call QueueGraphicsToLoad
	ret

INCLUDE "data/queued_tiledata/ruby_field/diglett.asm"

ResolveVoltorbCollision_RubyField: ; 0x14d85
	ld a, [wWhichVoltorb]
	and a
	jr z, .noVoltorbCollision
	xor a
	ld [wWhichVoltorb], a
	call ApplyVoltorbCollision_RubyField
	ld a, $10
...
ApplyVoltorbCollision_RubyField: ; 0x14dc9
	ld a, $ff
	ld [wRumblePattern], a
	ld a, $3
	ld [wRumbleDuration], a
...
ResolveRubyStageSpinnerCollision: ; 0x14dea
	ld a, [wSpinnerCollision]
	and a
	jr z, UpdateRubyStageSpinner
	xor a
...
UpdateRubyStageSpinner: ; 0x14e10
...
.asm_14e66
	...
	jr nz, .asm_14e8a
	call PlaySpinnerChargingSoundEffect_RubyField
	ret
...
PlaySpinnerChargingSoundEffect_RubyField: ; 0x14ea7
	ld a, [wd51e]
	and a
	ret nz
	ld a, [wPikachuSaverCharge]
	ld c, a
	ld b, $0
	ld hl, SpinnerChargingSoundEffectIds_RubyField
	add hl, bc
	ld a, [hl]
	ld e, a
	ld d, $0
...
SpinnerChargingSoundEffectIds_RubyField:
	db $12, $13, $14, $15, $16, $17, $18, $19, $1A, $1B, $1C, $1D, $1E, $1F, $20, $11

UpdateSpinnerChargeGraphics_RubyField: ; 0x14ece
...
INCLUDE "data/queued_tiledata/ruby_field/spinner.asm"

ResolveCAVELightCollision_RubyField: ; 0x151cb
	...
	inc hl
	and [hl]
	jr z, LoadCAVELightsGraphics_RubyField
	ld a, $1
	...
	call Increment_Max100
	jr LoadCAVELightsGraphics_RubyField
...
LoadCAVELightsGraphics_RubyField: ; 0x1522d
...
.loop
	ld a, [hld]
	push hl
	call LoadCAVELightGraphics_RubyField
	pop hl
	dec b
	jr nz, .loop
	ret

LoadCAVELightGraphics_RubyField: ; 0x1523c
...
UpdateCAVELightsBlinking_RubyField: ; 0x15270
	ld a, [wCAVELightsBlinking]
	and a
	jr z, .notBlinking
...
INCLUDE "data/queued_tiledata/ruby_field/cave_lights.asm"

ResolveBallUpgradeTriggersCollision_RubyField: ; 0x1535d
	...
	lb de, $00, $09
	call PlaySoundEffect
	jp LoadPinballUpgradeTriggersGraphics_RubyField
...
.done
	...
	jr LoadPinballUpgradeTriggersGraphics_RubyField
...
.loop
	ld a, [hld]
	push hl
	call LoadPinballUpgradeTriggerGraphics_RubyField
	pop hl
	dec b
	jr nz, .loop
	ret

LoadPinballUpgradeTriggerGraphics_RubyField: ; 0x15465
...
.load
	...
	inc hl
	ld b, [hl]
	ld a, Bank(TileDataPointers_15511_RubyField)
	ld de, LoadTileLists
	call QueueGraphicsToLoadWithFunc
	pop bc
	ret
...
UpdatePinballUpgradeBlinkingAnimation_RubyField: ; 0x154a9
	ld a, [wBallUpgradeTriggersBlinking]
	and a
	jr z, .notBlinking
...
BallTypeDegradationRubyField: ; 0x1550b
...
INCLUDE "data/queued_tiledata/ruby_field/ball_upgrade_triggers.asm"

UpdateBallTypeUpgradeCounter_RubyField: ; 0x15575
	ld a, [wCapturingMon]
	and a
	ret nz
...
ResolveRubyStageBoardTriggerCollision: ; 0x1581f
...
ResolveBellsproutCollision_RubyField: ; 0x15e93
	...
	lb de, $00, $05
	call PlaySoundEffect
	ld hl, BellsproutAnimationData_RubyField
...
BellsproutAnimationData_RubyField: ; 0x15f69
...
ResolveBumpersCollision_RubyField: ; 0x15f86
	ld a, [wWhichBumper]
	and a
	jr z, .noNewCollision
	call LoadBumpersGraphics_RubyField
	call LightUpBumper_RubyField
	xor a
	ld [wWhichBumper], a
	call ApplyBumperCollision_RubyField
.noNewCollision
	...
	call z, LoadBumpersGraphics_RubyField
	ret

LightUpBumper_RubyField: ; 0x15fa6
; Makes the hit bumper light up briefly
	...
	sla a
	inc a
	jr LoadBumperGraphics_RubyField

LoadBumpersGraphics_RubyField: ; 0x15fb8
...
LoadBumperGraphics_RubyField: ; 0x15fc0
	sla a
	ld c, a
	ld b, $0
...
.asm_15fd0
	...
	ld h, [hl]
	ld l, a
	ld a, Bank(TileDataPointers_16010_RubyField)
	call QueueGraphicsToLoad
	ret

ApplyBumperCollision_RubyField: ; 0x15fda
	ld a, $ff
	ld [wRumblePattern], a
	ld a, $3
	ld [wRumbleDuration], a
	...
	ld c, a
	ld b, $0
	ld hl, BumperCollisionAngleDeltas_RubyField
	add hl, bc
...
BumperCollisionAngleDeltas_RubyField:
	db -8, 8

INCLUDE "data/queued_tiledata/ruby_field/bumpers.asm"

ResolveDittoSlotCollision_RubyField: ; 0x160f0
...
ApplySlotForceField_RubyFieldBottom: ; 0x161af
; Applies the force field to the pinball when near the slot cave opening.
	...
	sla c
	add hl, bc
	jr _ApplySlotForceField_RubyField

ApplySlotForceField_RubyFieldTop: ; 0x161e0
; Applies the force field to the pinball when near the slot cave opening.
...
_ApplySlotForceField_RubyField: ; 0x1620f
; Applies the force field to the pinball when near the slot cave opening.
...
ResolveSlotCollision_RubyField: ; 0x16279
	ld a, [wSlotCollision]
	and a
	jr z, .noCollision
	xor a
...
.asm_16389
	...
	ld c, a
	ld b, $0
	ld hl, BonusStages_RubyField
	add hl, bc
	ld a, [hl]
	ld [wNextStage], a
	call ShowScrollingGoToBonusText_RubyField
	xor a
...
ShowScrollingGoToBonusText_RubyField: ; 0x163f2
	call FillBottomMessageBufferWithBlackTile
	call EnableBottomText
...
.asm_16441
	...
	or h
	ret z
	ld a, Bank(TileDataPointers_1644d_RubyField)
	call QueueGraphicsToLoad
	ret

INCLUDE "data/queued_tiledata/ruby_field/slot_cave.asm"

OpenSlotCave_RubyField: ; 0x164e3
	ld a, [wFramesUntilSlotCaveOpens]
	and a
	ret z
	dec a
...
ResolveRubyStagePinballLaunchCollision: ; 0x1652d
...
.notLaunchedYet
	...
	call ChooseInitialMap_RubyField
	ld a, $1
...
ResolveRubyStagePikachuCollision: ; 0x1660c
	ld a, [wWhichPikachu]
	and a
	jr z, .asm_1667b
	xor a
	...
	jr nz, .asm_16667
.asm_16634
	ld hl, PikachuSaverAnimationData_RubyField
...
.asm_16667
	ld hl, PikachuSaverAnimation2Data_RubyField
	ld de, wPikachuSaverAnimation
	call InitAnimation
	ld a, $2
	ld [wd51c], a
...
.asm_1667b
	ld a, [wd51c]
	and a
	call z, SetPikachuSaverSide_RubyField
	call UpdatePikachuSaverAnimation_RubyField
	ld a, [wPikachuSaverCharge]
	cp MAX_PIKACHU_SAVER_CHARGE
	ret nz
	ld a, [wd51e]
...
UpdatePikachuSaverAnimation_RubyField: ; 0x1669e
	ld a, [wd51c]
	cp $1
	jr nz, .asm_16719
	ld hl, PikachuSaverAnimationData_RubyField
...
PikachuSaverAnimationData_RubyField: ; 0x1673c
; Each entry is [duration][OAM id]
	db $0C, $02
	db $05, $03
	db $05, $02
...
PikachuSaverAnimation2Data_RubyField: ; 0x16761
; Each entry is [duration][OAM id]
	db $0C, $02
	db $01, $00
	db $00

SetPikachuSaverSide_RubyField: ; 0x16766
...
.asm_169db
	...
	ld h, [hl]
	ld l, a
	ld a, Bank(TileDataPointers_169ed_RubyField)
	call QueueGraphicsToLoad
	ret

INCLUDE "data/queued_tiledata/ruby_field/arrow_indicators.asm"

ResolveRubyStageBonusMultiplierCollision: ; 016d9d
	ld a, [wWhichBonusMultiplierRailing]
	and a
	jp z, UpdateBonusMultiplierRailing_RubyField
	xor a
...
UpdateBonusMultiplierRailing_RubyField: ; 0x16e51
	call ShowBonusMultiplierMessage_RubyField ; only shows the scrolling message when appropriate
	ld a, [wd612]
	and a
	jr z, .asm_16e8f
	dec a
...
ShowBonusMultiplierMessage_RubyField: ; 0x16ef5
...
_LoadBonusMultiplierRailingGraphics_RubyField: ; 0x16f28
	...
	jr nz, .gameboyColor
	pop af
	call _LoadBonusMultiplierRailingGraphics_RubyField_Gameboy
	ret

.gameboyColor
	pop af
	call _LoadBonusMultiplierRailingGraphics_RubyField_GameboyColor
	ret

_LoadBonusMultiplierRailingGraphics_RubyField_Gameboy: ; 0x16f38
	push af
	res 7, a
	ld hl, wd60e
	cp $14
...
.asm_16f47
	...
	ld b, $0
	sla c
	ld hl, BonusMultiplierRailingTileDataPointers_16fc8_RubyField
	add hl, bc
	ld a, [hli]
	ld h, [hl]
	ld l, a
	ld a, Bank(BonusMultiplierRailingTileDataPointers_16fc8_RubyField)
	call QueueGraphicsToLoad
.asm_16f5c
	pop af
	ld bc, $0000
...
.asm_16f68
	...
	set 2, c
.asm_16f6e
	ld hl, BonusMultiplierRailingTileData_171e4_RubyField
	add hl, bc
	ld a, [hli]
	ld h, [hl]
	ld l, a
	ld a, Bank(BonusMultiplierRailingTileData_171e4_RubyField)
	call QueueGraphicsToLoad
	ret

_LoadBonusMultiplierRailingGraphics_RubyField_GameboyColor: ; 0x16f7b
	bit 7, a
	jr z, .asm_16f83
	res 7, a
	add $a
.asm_16f83
	...
	ld b, $0
	sla c
	ld hl, BonusMultiplierRailingTileDataPointers_17228_RubyField
	add hl, bc
	ld a, [hli]
	ld h, [hl]
	ld l, a
	ld a, Bank(BonusMultiplierRailingTileDataPointers_17228_RubyField)
	call QueueGraphicsToLoad
	ret

INCLUDE "data/queued_tiledata/ruby_field/bonus_multiplier_railings.asm"

UpdatePokeballs_RubyField: ; 0x174d0
; Update the pokeballs underneath the billboard, which blink for awhile after catch'em mode and evolution mode.
	call UpdateBlinkingPokeballs_RubyField
	ret nc
	; fall through
LoadPokeballsGraphics_RubyField: ; 0x1f265
; Loads the graphics for the list of pokeballs underneath the billboard picture.
	sla a
	ld c, a
	ld b, $0
	ld hl, TileDataPointers_1f2b9_RubyField
	add hl, bc
	ld a, [hli]
	ld c, a
	ld a, [hli]
	ld b, a
	ld a, Bank(TileDataPointers_1f2b9_RubyField)
	ld de, LoadTileLists
	call QueueGraphicsToLoadWithFunc
	ret

UpdateBlinkingPokeballs_RubyField: ; 0x174ea
	ld a, [wPreviousNumPokeballs]
	ld hl, wNumPokeballs
	cp [hl]
	ret z
